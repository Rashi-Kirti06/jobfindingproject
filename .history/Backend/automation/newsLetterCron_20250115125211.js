        ...
        for(const job of jobs){
            try {
                ...
                for(const user of filterUsers){
                    ...
                    const message = `Hi ${user.name}, \n\n Great News! A new job that fits your niche has just been posted. the position is for a ${job.title} with ${job.companyName}, and they are looking to hire immendiatly`;
                    sendEmail({
                        email: user.email,
                        subject,
                        message
